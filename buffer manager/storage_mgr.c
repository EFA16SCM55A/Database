...
extern RC readBlock(int pageNum, SM_FileHandle *fHandle, SM_PageHandle memPage) {
	...
	size_t t = fread((void*)memPage, 1, PAGE_SIZE, thisFile);
	if (t == PAGE_SIZE) {
...
// made minor changes to to code to test
...
extern RC appendEmptyBlock(SM_FileHandle *fHandle) {
	...
	if (fHandle == NULL) {
		...
		int rc=0;
		if (s != 0) {
			//printf("returning with error from appendEmptyBlock\n");
			return RC_READ_NON_EXISTING_PAGE;
		}
		else {
			// Write an empty page at the end of the file
			// initialize the new page with NULL values.
			//w = fwrite(&data, 1, 4096, f);
			rc=fwrite((const char*)data,sizeof(data),1,fHandle->mgmtInfo);
			//printf("Appended %d empty bytes.", w);
			//printf("rc = %d\n", rc);
			if (rc != 1) {
				//printf("returning with error from appendEmptyBlock 2\n");
				return RC_WRITE_FAILED;
			}
			else {
	...
	if (fHandle != NULL) {
		...
		if (filepages<numberOfPages) {
			...
			for (i = 0; i<increasepages; i++) {
				//printf("Calling appendEmptyBlock %d\n", i);
