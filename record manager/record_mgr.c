...
// startScan scans all the records in the table based on a condition and returns the output
extern RC startScan (RM_TableData *rel, RM_ScanHandle *scan, Expr *cond)
{
	...
	// Allocating memory to the ManagementData structure
	...
	// we start the scan with page 1 in the buffer pool because page0 has table information
    	scanManager->recordID.page = 1;
    	
	// 0 to start scan from the first slot	
	scanManager->recordID.slot = 0;
	
	scanManager->scanCount = 0;//counts the number of tuples scanned
...
extern RC next (RM_ScanHandle *scan, Record *record)
{
	...
	// Calculating Total number of slots for records in a page
...
extern RC getAttr (Record *record, Schema *schema, int attrNum, Value **value)
{
	...
	// Calculating the offset value of each attribute
	attrOffset(schema, attrNum, &offset);

	// Allocating memory space to store the attribute values
	...
	// unserializing function implemented here
